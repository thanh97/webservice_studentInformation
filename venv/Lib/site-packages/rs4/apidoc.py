...
class API:
  ...
  def argslist (self, part):
    if self.parameter_requirements.get (part):
      self.writeln ("**Required {} Parameters**:".format (part))
      for arg in self.parameter_requirements [part][0]:
  ...
  def render (self, out):
    ...
    self.writeln ('**Endpoint(s)**: {}'.format (self.chiplist (list (self.endpoints))))
    ...
    qss = self.parameter_requirements.get ("ARGS", [[]]) [0]
    if not qss:
      qss = self.routeopt ['args'][self.routeopt.get ('urlargs', 0):]
    if qss:
      self.writeln ("**Method-Wide Common Parameters**:")
      ...
      for status_code in sorted (self.METHODS [method].keys ()):
        for url, reason, reqh, reqd, resh, resd, spec in self.METHODS [method][status_code]:
          if 200 <= status_code < 300:
            ...
            if idx [0] >= 3:
              continue
            self.writep ("**Sucess Response Example**")
...
def build_doc ():
  if not os.path.isdir (".webtest_log/v"):
    print ("apidoc.build_doc: no log directory, skipped.")
    return
  ...
  #with sys.stdout as out:
    ...
    sorted_apis = sorted (apis.items (), key = lambda x: x [0])
    for idx, (resource_id, api) in enumerate (sorted_apis):
      ...
      for method in API.VALID_METHODS:
        ...
        try:
          ...
          methods_count [method] = 0
          out.write (" [**`{}`**](#{})".format (method, method.lower ()))
        else:
          out.write (" [**`{}`**](#{}-{}) ".format (method, method.lower (), methods_count [method]))
...
def log_spec (method, url, status_code, reason, reqh, reqd, resh, resd):
    ...
    with open (os.path.join (".webtest_log/v", '{:04d}.spec'.format (SPECS)), 'wb') as f:
        pickle.dump ((method, url, status_code, reason, reqh, reqd, resh, resd, spec), f)
