...
class file_producer:
	...
	def __init__ (self, file, buffer_size = SIZE_BUFFER, proxsize = -1, offset = 0, limit = 0):
		self.done = 0
		self.file = file
		self.buffer_size = buffer_size
		...
		self.limit = limit
		self.partial = self.limit and True or False
...
class escaping_producer:
	...
	def more (self):
		...
		if buffer:
			...
			i = asynchat.find_prefix_at_end (buffer, esc_from)
			if i:
				# we found a prefix
